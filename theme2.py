...
TERMUX = H / ".termux"
BIN = H / "bin"
FISH = H / ".config/fish/conf.d"
FONTS = H / ".termux-themes/fonts"
BACKUP = H / ".termux-backup"
SCRIPT = H / "termux-theme.py"
PROMPT = FISH / "90-elmy0711.fish"

RAW = "https://raw.githubusercontent.com/minority1001/theme/main/install-theme.py"
FONT_URL = "https://github.com/ryanoasis/nerd-fonts/releases/latest/download/{}.tar.xz"

THEMES = {
    "1": ("Tokyo Night", "JetBrainsMono",
          "#1a1b26", "#a9b1d6",
    ...
    "4": ("Gruvbox", "CascadiaCode",
          "#282828", "#ebdbb2",
          ["#282828","#cc241d","#98971a","#d79921","#458588","#b16286","#689d6a","#a89984",
           "#928374","#fb4934","#b8bb26","#fabd2f","#83a598","#d3869b","#8ec07c","#ebdbb2"]),

    "5": ("Catppuccin", "Iosevka",
          "#1e1e2e", "#cdd6f4",
...
def setup():
    for p in (TERMUX, BIN, FISH, FONTS):
...
def font(name):
    ...
    if not f:
        a = d/f"{name}.tar.xz"
        try:
            urllib.request.urlretrieve(FONT_URL.format(name), a)
    ...
    if f:
        shutil.copy2(f, TERMUX/"font.ttf")
...
def apply(n):
    ...
    backup()
    (TERMUX/"colors.properties").write_text(colors(c))
    fish(c)
    keyboard()
    command()
    font(c[1])
...
def install():
    ...
    print("\n╭────────────────────────────╮")
    print("│ ELMY0711 INSTALL SELESAI   │")
    print("├────────────────────────────┤")
    print("│ Command : tema             │")
    print("│ Prompt  : ELMY0711         │")
...
def main():
    ...
    if len(__import__("sys").argv) > 1:
        if __import__("sys").argv[1] in ("-i", "--install"):
            install()
            return
