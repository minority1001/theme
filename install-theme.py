...
FONTDIR = HOME / ".termux-themes/fonts"
...
def setup():
    for path in (TERMUX, BIN, FISH, FONTDIR):
...
def install_font(name):
    ...
    font = next(folder.rglob("*.ttf"), None)
    ...
    if font:
        shutil.copy2(font, TERMUX / "font.ttf")
...
def apply_theme(number):
    ...
    print("✓ config.fish tidak disentuh")
    print("✓ Jalankan: exec fish")


def restore():
    if not BACKUP.exists():
        print("Backup belum tersedia.")
        return
...
def create_main_script():
    ...
    try:
        urllib.request.urlretrieve(RAW, MAIN)
    except Exception:
        print("! Gagal membuat ~/termux-theme.py")
